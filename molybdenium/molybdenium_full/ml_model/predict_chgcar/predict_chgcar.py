...
jl = JLPredictor(jl_settings=settings, model_path=f"../train_ml/scikit_{model_name}.p", grid_size=(160,160,160))
...
for frame in tqdm(frames,total=nframes):
    ...
    t_init = time.time()
    ml_chg = jl.predict_chgcar(atoms,nelect=756,batch_size=batch_size,verbose=False,save_path=save_path,name=f'CHGCAR_{frame}',return_chg=True,write_chgcar=True)
