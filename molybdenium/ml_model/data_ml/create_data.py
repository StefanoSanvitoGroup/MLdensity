...
frames_dict = {'train': np.arange(10,dtype=int), 'test': np.arange(10,20,dtype=int)}
n_samples_per_snapshot = 5000
...
with tqdm(total=sum([len(frames_dict[partition]) for partition in ['train','test']])) as pbar:
    for partition in ['train','test']:
        for frame in frames_dict[partition]:
            ...
            t_init = time.time()
            atoms = read(scf_path + f"/{partition}_scf/{frame}/POSCAR")
            chgcar = Chgcar.from_file(scf_path + f"/{partition}_scf/{frame}/CHGCAR")
