...
jl = JLPredictor(jl_settings=settings, model_path=f"../train_ml/scikit_{model_name}.p", grid_size=(180,180,180))
...
for frame in tqdm(range(nframes)):
    ...
    chgcar = Chgcar.from_file(f"{scf_path}/test_scf/{frame}/CHGCAR")
    ...
    t_init = time.time()
    ml_chg = jl.predict_chgcar(atoms,nelect=30,batch_size=batch_size,verbose=False,save_path=save_path,name=f'CHGCAR_{frame}',return_chg=True,write_chgcar=True)
    ...
    with open(f"{save_path}/metrics_test.out", "a") as logfile:
        logfile.write(f"   {frame:>4d}    |  {test_r2: 8.6e}  |  {test_rmse: 8.6e}  |  {test_mae: 8.6e}  |  {test_maxae: 8.6e}  |  {test_mse: 8.6e}  \n")
