...
frames = np.sort(np.asarray(['1Tprime_mos2-'+filename.split('/')[-2] for filename in glob.glob(scf_path+'/test_scf/1Tprime_mos2/*/CHGCAR')]))
nframes = len(frames)
batch_size = 100000
...
jl = JLPredictor(jl_settings=settings, model_path=f"../train_ml/scikit_{model_name}.p", grid_size=(216,192,300))
# jl = JLPredictor(jl_settings=settings, model_path=f"../train_ml/scikit_{model_name}.p", encut=600.0,prec='Accurate')
...
for frame in tqdm(frames,total=nframes):
    ...
    t_init = time.time()
    atoms = read(f"{scf_path}/test_scf/{frame.replace('-','/')}/POSCAR")
    chgcar = Chgcar.from_file(f"{scf_path}/test_scf/{frame.replace('-','/')}/CHGCAR")
    ...
    t_init = time.time()
    ml_chg = jl.predict_chgcar(atoms,nelect=416,batch_size=batch_size,verbose=False,save_path=save_path,name=f'CHGCAR_{frame}',return_chg=True,write_chgcar=True)
    ...
    with open(f"{save_path}/metrics_test_{frame}.out", "w") as logfile:
        logfile.write("           |        R2       |      RMSE       |       MAE       |      MaxAE      |       MSE     \n")
        logfile.write(f"Test       |  {test_r2: 8.6e}  |  {test_rmse: 8.6e}  |  {test_mae: 8.6e}  |  {test_maxae: 8.6e}  |  {test_mse: 8.6e}  \n")

    if not os.path.exists(f"{save_path}/metrics_test.out"):
        with open(f"{save_path}/metrics_test.out", "w") as logfile:
            logfile.write("           |        R2       |      RMSE       |       MAE       |      MaxAE      |       MSE     \n")

    with open(f"{save_path}/metrics_test.out", "a") as logfile:
        logfile.write(f"   {frame}    |  {test_r2: 8.6e}  |  {test_rmse: 8.6e}  |  {test_mae: 8.6e}  |  {test_maxae: 8.6e}  |  {test_mse: 8.6e}  \n")

    plot_corr_scatter(r"CHG (e/$\rm \AA^3$)",dft_chg,ml_chg,savefig=True,save_path="figures",model_name=f"test_{frame}",show=True)

    t_init = time.time()
    atoms.write(f"poscar_files/POSCAR_{frame}", vasp5=True, sort=True)

    ml_chgcar = Chgcar.from_file(f"{save_path}/CHGCAR_{frame}")
    ml_chgcar.data_aug['total'] = chgcar.data_aug['total']
    ml_chgcar.write_file(f"{save_path}/CHGCAR_{frame}")
