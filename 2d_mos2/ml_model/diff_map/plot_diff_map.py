...
frame = '1Tprime_mos2-15'

atoms = read(scf_path + f"/test_scf/{frame.replace('-','/')}/POSCAR")
vol = atoms.get_volume()
chgcar = Chgcar.from_file(scf_path + f"/test_scf/{frame.replace('-','/')}/CHGCAR")
...
imshow_ax = ax.imshow(chg_dft_points.reshape((ngxf,ngyf),order='C').T,cmap='cividis',origin='lower')
...
fig.tight_layout()
fig.savefig('figures/'+f"dft_chg_{frame.replace('-','_')}.png",dpi=300,bbox_inches='tight')
plt.show()

fig, ax = plt.subplots(figsize=(6,6),dpi=100)
imshow_ax = ax.imshow(chg_pred_points.reshape((ngxf,ngyf),order='C').T,cmap='cividis',origin='lower')
ax.set_xlim((0,ngxf-1))
ax.set_ylim((0,ngyf-1))
ax.set_xticks([tick for tick in range(0,ngxf,20)])
ax.set_yticks([tick for tick in range(0,ngyf,20)])
...
fig.tight_layout()
fig.savefig('figures/'+f"ml_chg_{frame.replace('-','_')}.png",dpi=300,bbox_inches='tight')
plt.show()

fig, ax = plt.subplots(figsize=(6,6),dpi=100)
imshow_ax = ax.imshow(chg_diff_points.reshape((ngxf,ngyf),order='C').T,cmap='seismic',origin='lower', vmin=vmin, vmax=vmax)
ax.set_xlim((0,ngxf-1))
ax.set_ylim((0,ngyf-1))
ax.set_xticks([tick for tick in range(0,ngxf,20)])
ax.set_yticks([tick for tick in range(0,ngyf,20)])
...
fig.tight_layout()
fig.savefig('figures/'+f"diff_chg_{frame.replace('-','_')}.png",dpi=300,bbox_inches='tight')
plt.show()
...
test_rmse = metrics.root_mean_squared_error(chg_dft_points,chg_pred_points)
